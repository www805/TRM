...
public interface Police_problemMapper extends BaseMapper<Police_problem> {
    @Select("select DISTINCT(p.id),p.*,pp.problemtypessid from police_problem p LEFT JOIN police_problemtotype pp  ON pp.problemssid=p.id where 1=1 ${ew.sqlSegment}" )
