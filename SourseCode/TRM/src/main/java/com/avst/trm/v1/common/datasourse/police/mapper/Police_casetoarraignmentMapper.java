...
public interface Police_casetoarraignmentMapper extends BaseMapper<Police_casetoarraignment> {
    ...
    @Select("select a.*,r.recordname,r.recordbool,r.recordtime ,au1.username as adminname ,au2.username as recordadminname,r.id as recordssid " +
            " from police_arraignment a " +
            " left join police_casetoarraignment cr on cr.arraignmentssid=a.id " +
            " left join base_admininfo au1 on au1.id=a.adminssid " +
            " left join base_admininfo au2 on au2.id=a.recordadminssid " +
            " left join police_record r on r.id=a.recordssid where 1=1 ${ew.sqlSegment}")
